...
Let’s look at the sudoku package (yep, someone wrote a program to create sudoku puzzles in R). To get it we type: `install.packages( "sudoku" )`. To load the package we use the `library()` function: `library( sudoku )`. Let’s take a look at what the package has: `help( package="sudoku" )`.
