...
## **Example Data File**  
Let's work through an example. Start by downloading the "R_Workshop_data.csv" file from this [link](r_workshop_data.csv) on the [RWorkshop website](https://jacobtnyoung.github.io/RWorkshop/).  
...
dat <- read.csv( "r_workshop_data.csv", header = TRUE, as.is = TRUE, na.strings = "." )
dat
```  
...
```{r,echo=TRUE,eval=FALSE}
dat <- read.csv(
    "https://raw.githubusercontent.com/jacobtnyoung/RWorkshop/main/r_workshop_data.csv",
...
dat,1:2] #same thing as data[,c(1,2)].
...
dat.c <- merge( data.a,data.b,by.x = "id" )
dat.d <- merge( data.a,data.b,by.x = "id",all.x=TRUE ) #note difference.
```
<br>
