...
## **Example Data File**  
Let's work through an example. Start by taking a look at the "R_Workshop_data.csv" file from this [link](https://github.com/jacobtnyoung/RWorkshop/blob/master/r_workshop_data.csv) on the [RWorkshop website](https://github.com/jacobtnyoung/RWorkshop). 

Now, we need to save this file to our desktop. To do so, right click the "Raw" button as shown in the image below:  
![file call image](images/csv.1.png)  
<br>
  
Then, change the file format and save it as shown below:  
![file call image](images/csv.2.png)  
<br>  
  
Open it and take a look at what is in the file. The file contains 52 individuals and 4 variables. The variables are: respondents id ("id"), a binary variable indicating whether the respondent is male or female ("male" where "1" is male), a measure of the respondent’s age ("age"), and a measure of risky behaviors engaged in by the respondent ("risky").  
<br>
...
In RStudio, you can go to the "Files" tab and look up the file.   

<br>

Note that we could *skip* the step of storing the .csv file locally and simply call file from the website url.   
For example:
```{r,echo=TRUE,eval=FALSE}
data <- read.csv(
    "https://github.com/jacobtnyoung/RWorkshop/raw/master/r_workshop_data.csv",
